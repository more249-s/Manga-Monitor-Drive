  ...
  .addSubcommand((s) =>
    s
      ...
      .addStringOption((o) =>
        o
          .setName("source_url")
          .setDescription("Source URL for tracking")
          .setRequired(true),
      )
      .addStringOption((o) =>
        o
      ...
      .addNumberOption((o) =>
        o
          .setName("current_raw")
          .setDescription("Current RAW chapter")
          .setRequired(false),
      )
      .addNumberOption((o) =>
        o
          .setName("chapter_payment")
          .setDescription("Payment per chapter ($)")
          .setRequired(false),
      ),
  )
  .addSubcommand((s) =>
  ...
  .addSubcommand((s) =>
    s
      ...
      .addStringOption((o) =>
        o
          .setName("source_url")
          .setDescription("New source URL")
          .setRequired(true),
      )
      .addStringOption((o) =>
        o
          .setName("source_name")
          .setDescription("Source name (optional)")
          .setRequired(false),
      ),
  );
...
async function handleCreate(
  ...
  const chapterPayment =
    interaction.options.getNumber("chapter_payment") || 6.0;
  ...
  try {
    ...
    console.error("[Project] Failed to create channel/role:", err);
  }
  ...
  const dashboardEmbed = new EmbedBuilder()
    .setTitle(`📖 ${name}`)
    .setDescription(
      `${typeIcon} ${projectType === "exclusive" ? "Exclusive" : "Competitive"}`,
    )
  ...
  if (dashboardMsgId) {
    await pool.query(
      "UPDATE projects SET dashboard_message_id = $1 WHERE id = $2",
      [dashboardMsgId, projectId],
    );
  }
  ...
  const replyEmbed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "Channel",
        value: channel ? `<#${channel.id}>` : "Not created",
        inline: true,
      },
...
async function handleRemove(
  ...
  const { rows } = await pool.query("SELECT * FROM projects WHERE slug = $1", [
    slug,
  ]);
  if (!rows.length) {
    await interaction.reply({
      content: `No project found with slug \`${slug}\`.`,
      ephemeral: true,
    });
    return;
  }

  await pool.query("UPDATE projects SET status = 'archived' WHERE slug = $1", [
    slug,
  ]);
  await interaction.reply({
    content: `Project **${rows[0].name}** has been archived.`,
    ephemeral: true,
  });
}
...
async function handleInfo(
  ...
  const embed = new EmbedBuilder()
    .setTitle(`📖 ${p.name}`)
    .setDescription(
      `${typeIcon} ${p.project_type === "exclusive" ? "Exclusive" : "Competitive"}`,
    )
      ...
      { name: "Payment/Chapter", value: `$${p.chapter_payment}`, inline: true },
      {
        ...
        value:
          srcs
            .map(
              (s: any) =>
                `• [${s.source_name}](${s.source_url}) — Last: Ch.${s.last_chapter}`,
            )
            .join("\n") || "None",
        inline: false,
      },
...
async function handleSetSource(
  ...
  const sourceUrl = interaction.options.getString("source_url", true);
  const sourceName =
    interaction.options.getString("source_name") || new URL(sourceUrl).hostname;
  ...
  await interaction.reply({
    content: `Source **${sourceName}** added for project \`${slug}\`.`,
    ephemeral: true,
  });
}
