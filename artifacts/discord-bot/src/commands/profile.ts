...
export const data = new SlashCommandBuilder()
  ...
  .addUserOption((o) => o.setName("user").setDescription("Member to view (leave empty for yourself)").setRequired(false));

export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  const target = interaction.options.getUser("user") || interaction.user;

  const { rows } = await pool.query("SELECT * FROM members WHERE discord_id = $1", [target.id]);
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor("#0099ff")
    .addFields(
      { name: "Role", value: roleLabels[member.role] || member.role, inline: true },
      { name: "Rate", value: `$${member.chapter_rate}/chapter`, inline: true },
      { name: "Payment", value: member.payment_method || "Not set", inline: true },
      { name: "Total Chapters", value: `${member.total_chapters}`, inline: true },
      { name: "This Month", value: `${monthChapters} chapters`, inline: true },
      { name: "Monthly Earnings", value: `$${monthTotal.toFixed(2)}`, inline: true },
      { name: "Total Earned", value: `$${totalEarned.toFixed(2)}`, inline: true },
      {
        name: "Active Chapters",
        value:
          activeChapters.length > 0
            ? activeChapters.map((c: any) => `• ${c.project_name} Ch.${c.chapter_number} (${c.status})`).join("\n")
            : "None",
        inline: false,
      }
    )
